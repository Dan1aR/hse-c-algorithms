...
def run_tests(python_script: str, test_files_dir: str):
    test_files = sorted(f for f in os.listdir(test_files_dir) if os.path.isfile(os.path.join(test_files_dir, f)))
    ...
    for test_file in test_files:
        ...
        parts = re.split(r'\n\s*\n\s*\n', content, maxsplit=1)
        if len(parts) < 2:
            print(f"{RED}Файл {test_file} имеет неверный формат: не найден разделитель из трёх пустых строк.{RESET}")
            sys.exit(1)
        ...
        if result.returncode != 0:
            print(f"{RED}Тест {test_file} провален. Код возврата: {result.returncode}{RESET}")
            print("Вывод:")
            print(result.stdout)
            print("Ошибка:")
            print(result.stderr)
        ...
        if result.stdout.rstrip() != expected_output.rstrip():
            print(f"{RED}Тест {test_file} провален. Фактический вывод не совпадает с ожидаемым.{RESET}")
            print("Ожидалось:")
