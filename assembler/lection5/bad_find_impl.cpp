...
/*
    ...
    mmap - это просто выделение виртуальной памяти (страницы). но физически ниче не выделили
    ...
    Здесь важный момент ещё заключается в том, что аллокация идет за системным mmap-ом, чтобы
    получить память либо в случае, когда ей не хватает уже выделенных виртуальных страниц, либо 
    (наш кейс) когда выделяется какой-то здоровый кусок памяти (а у нас одна строка - 1Кб)
    и аллокатор не может отрезать кусочек из "арены", а честно идет в ОС делать mmap на каждую
    строку. Потому что мы проводили замер, когда там строки были маленькие, но больше и аллокатор
    спокойно выдавал всё из арены и в ОС почти не ходил
*/
